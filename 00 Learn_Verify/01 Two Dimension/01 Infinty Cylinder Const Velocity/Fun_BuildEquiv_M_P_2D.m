 ...
 *
 * @File:       Fun_BuildEquiv_M_P_2D.m
 * @Brief:      【二维问题】求解[系数矩阵]和[声压矩阵]
 * 
 * @Input:      FieldPoints                     检测场点坐标                        FieldPoints_Num × 2数组
 *              Equiv_Simple_Source_Coord       【等效简单源】X、Y坐标               M × 2数组
 *              k                               介质波数
 *              SoundVelocity                   介质声速
 *              Rho                             介质密度
 *              Equiv_Q                         等效源强度矩阵                      M × 1数组  
 * 
 * @Output:     Equiv_M                         系数矩阵
 *              Equiv_P                         声压矩阵
 * 
...
function [Equiv_P, Equiv_M] = Fun_BuildEquiv_M_P_2D(FieldPoints, Equiv_Simple_Source_Coord, k, SoundVelocity, Rho, Equiv_Q)
...
for ii = 1 : FieldPoints_Num
    r_ii = FieldPoints(ii, :);                                              % 声压监测点坐标
    for jj = 1 : Equiv_Simple_Source_Num
        r_vector = r_ii - [Equiv_Simple_Source_Coord(jj, 1), Equiv_Simple_Source_Coord(jj, 2)];     % 观测点矢量与等效源矢量差(新的向量)
        r_distance = norm(r_vector);                                        % 观测点与等效源的距离(新的向量)的模
        Equiv_M(jj, ii) = -1 * Omega * Rho * pi * besselh(0, k * r_distance);                                    % 系数矩阵M
    end
    Equiv_P(ii) = sum(Equiv_M(:, ii) .* Equiv_Q);
end
