 ...
 *
 * @File:       OscillatingSphereMonopole.m
 * @Brief:      1. 以【振荡球源】为研究对象，导入[节点坐标](分三种节点密度：58、87、123)
 *              2. 基于【点云处理】计算得到节点[外法向量]，并进行可视化
 *              3. 基于【解析式】求解【脉动球源】辐射声压(距离、波数)
 *              4. 基于【波叠加法】求解【振荡球源】辐射声压(距离、波数)
...
% 结构信息
Sphere_Radius = 1;                                                                                      % 【球源】直径
Velocity_Amplitude = 1;                                                                                 % 表面振速幅值
OriginPoint_NormalVelocity_Array = ones(length(OriginPoint_Num_Array), 1) * Velocity_Amplitude;
for i = 1 : length(OriginPoint_Coord_Array)
    r_i = norm(OriginPoint_Coord_Array(i, :));
    cos_theta_i = dot(OriginPoint_Coord_Array(i, :), [1 0 0]) / r_i;
    OriginPoint_NormalVelocity_Array(i) = OriginPoint_NormalVelocity_Array(i) * cos_theta_i;         % 【球源】离散节点 [速度幅值]
end
...
hold on;
quiver3(ptCloud_X, ptCloud_Y, ptCloud_Z, Scale_Arrows * Normal_X, Scale_Arrows * Normal_Y, Scale_Arrows * Normal_Z);
hold off;
...
for i = 1 : FieldPoints_Num                                                                             % 遍历各个测点
    r_i = norm(FieldPoints(i, :));                                                                      % 测点与【球源】球心的距离
    cos_theta_i = dot(FieldPoints(i, :), [1 0 0]) / r_i;                                                % 测点与振荡方向 X轴(正向) 的夹角余弦值
    P_Distance(i) = - (Velocity_Amplitude * cos_theta_i) * (1i * k_Origin * AcousticVelocity_Medium * Density_Medium * Sphere_Radius^3 * (1 - 1i * k_Origin * r_i) / (2 * (1 - 1i * k_Origin * Sphere_Radius) - (k_Origin * Sphere_Radius)^2)) * exp(1i * k_Origin * (r_i - Sphere_Radius)) / r_i^2;
%     P_Distance(i) = - (Velocity_Amplitude * cos_theta_i) * 1i * k_Origin * Density_Medium * AcousticVelocity_Medium * Sphere_Radius * (1 - 1i * k_Origin * Sphere_Radius) * exp(1i * k_Origin * (r_i - Sphere_Radius)) / (2 * (1 - 1i * k_Origin * Sphere_Radius) - (k_Origin * Sphere_Radius)^2);
end
...
Fun_MultiPlot2(1, FieldPoints(:, 1), abs(P_Distance), '离球心的距离 (m)', '声压幅值 (Pa)', '图2.2 【解析解】结构辐射声压幅值(距离)', false, min(FieldPoints(:, 1)), max(FieldPoints(:, 1)));
subplot(2, 2, 3);                                                                                       % 图(2.3) 【解析解】结构辐射声压实部(距离)
Fun_MultiPlot2(1, FieldPoints(:, 1), real(P_Distance), '离球心的距离 (m)', '实部声压 (Pa)', '图2.3 【解析解】结构辐射声压实部(距离)', false, min(FieldPoints(:, 1)), max(FieldPoints(:, 1)), 'n', 'n', 'n', '#8b7d6b', 'n');
subplot(2, 2, 4);                                                                                       % 图(2.4) 【解析解】结构辐射声压虚部(距离)
Fun_MultiPlot2(1, FieldPoints(:, 1), imag(P_Distance), '离球心的距离 (m)', '虚部声压 (Pa)', '图2.4 【解析解】结构辐射声压虚部(距离)', false, min(FieldPoints(:, 1)), max(FieldPoints(:, 1)), 'n', 'n', 'n', '#5f9ea0', 'n'); 
...
cos_theta_i = dot(Monitor_Point, [1 0 0]) / r_i;                                                        % 测点与振荡方向 X轴(正向) 的夹角余弦值
for i = 1 : length(k_Range)
    k = k_Range(i);
    P_kRange(i) = - (Velocity_Amplitude * cos_theta_i) * (1i * k * AcousticVelocity_Medium * Density_Medium * Sphere_Radius^3 * (1 - 1i * k * r_i) / (2 * (1 - 1i * k * Sphere_Radius) - (k * Sphere_Radius)^2)) * exp(1i * k * (r_i - Sphere_Radius)) / r_i^2;
end
...
% 基于[法向振速]正求[源强度矢量]
[Equiv_D_Radiation, Equiv_Q_Radiation] = Fun_BuildEquiv_D_Q(OriginPoint_Coord_Array, EquivSource_Coord_Array, k_Origin, OriginPoint_NormalVelocity_Array, OriginPoint_Normals_Array);
% 基于[源强度矢量]正求节点[辐射声压]
...
hold on;
Fun_MultiPlot2(1, FieldPoints(:, 1), abs(Equiv_P_Radiation), '离球心的距离 (m)', '声压幅值 (Pa)', '图5.2 结构辐射声压幅值对比(距离)', false, min(FieldPoints(:, 1)), max(FieldPoints(:, 1)), 'n', 'n', '--', 'r', 1.5);
hold off;

subplot(2, 2, 3);                                                                                       % 图(5.3) 结构辐射声压实部对比(距离)
plot(FieldPoints(:, 1), real(P_Distance), 'Color', '#8b7d6b', 'LineWidth', 1.5);
hold on;
Fun_MultiPlot2(1, FieldPoints(:, 1), real(Equiv_P_Radiation), '离球心的距离 (m)', '实部声压 (Pa)', '图5.3 结构辐射声压实部对比(距离)', false, min(FieldPoints(:, 1)), max(FieldPoints(:, 1)), 'n', 'n', '--', 'r', 1.5);
hold off;
 
subplot(2, 2, 4);                                                                                       % 图(5.4) 结构辐射声压虚部对比(距离)
plot(FieldPoints(:, 1), imag(P_Distance), 'Color', '#5f9ea0', 'LineWidth', 1.5);
hold on;
Fun_MultiPlot2(1, FieldPoints(:, 1), imag(Equiv_P_Radiation), '离球心的距离 (m)', '虚部声压 (Pa)', '图5.4 结构辐射声压虚部对比(距离)', false, min(FieldPoints(:, 1)), max(FieldPoints(:, 1)), 'n', 'n', '--', 'r', 1.5);
hold off;
...
for i = 1 : length(k_Range)
    k = k_Range(i);
    [Equiv_D_Radiation, Equiv_Q_Radiation] = Fun_BuildEquiv_D_Q(OriginPoint_Coord_Array, EquivSource_Coord_Array, k, OriginPoint_NormalVelocity_Array, OriginPoint_Normals_Array);
