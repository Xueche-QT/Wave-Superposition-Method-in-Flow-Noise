...
function Equiv_P = Fun_BuildEquiv_M_P(FieldPoints, Equiv_Simple_Source_Coord, k, SoundVelocity, Rho, Equiv_Q)
...
for i = 1 : FieldPoints_Num
    ...
    Equiv_M = zeros(Equiv_Simple_Source_Num, 1);                            % 初始化【系数矩阵】
    for j = 1 : Equiv_Simple_Source_Num
        ...
        Equiv_M(j) = - 1i * Omega * Rho * exp(1i * k * r_distance) / (4 * pi * r_distance);                                               % 系数矩阵M
    end
    Equiv_P(i) = sum(Equiv_M .* Equiv_Q);
end
