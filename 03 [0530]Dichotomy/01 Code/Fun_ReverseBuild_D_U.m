 ...
 *              EquivReverse_Q                  
 * 
 * @Output:     EquivReverse_D                  声偶极矩阵
 *              EquivReverse_U_normal           源强度矩阵
 * 
 * @Author:     Haiger
 * @date:       2023.05.19
 *------------------------------------------------------------------------------------------
...
function [EquivReverse_D,EquivReverse_U_normal] = Fun_ReverseBuild_D_U(Origin_Discrete_Node_Coord, Equiv_Simple_Source_Coord, k, EquivReverse_Q, Origin_Discrete_Node_Normal)
...
EquivReverse_D = zeros(Equiv_Simple_Source_Num, Origin_Discrete_Node_Num);         % 初始化【声偶极矩阵】
...
%         theta_ij = acos(dot(r_difference, normal_j) / r_distance);          % 位置矢量差(新的向量)与【结构源】表面第 j 个离散节点法向向量的夹角
        ...
        EquivReverse_D(i,j) = - 1 / (4 * pi) * (1j * k * r_distance - 1) / r_distance^2 * exp(1j * k * r_distance) * cos(theta_ij);    % 声偶极矩阵
    end
end

EquivReverse_U_normal = EquivReverse_D \ EquivReverse_Q;
end